...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let spanThresholdWidth = (singleSpanWidth + doubleSpanWidth) / 2; 
  ...
  function computeCols() {
    ...
    spanThresholdWidth = (singleSpanWidth + doubleSpanWidth) / 2;
  ...
  function layout(skipEl = null) {
    ...
    items.forEach((item) => {
      ...
      const width = span === 1 ? cellWidth : doubleSpanWidth;
  ...
  function updateGestureSize() {
      ...
      const maxWidth = gestureItem.group === 'left' ? doubleSpanWidth : cols * (cellWidth + gap) - gap;
  ...
  function handleMove(e) {
    ...
    if (resizingItem) {
      ...
      if (resizingItem.group === 'left') {
        maxWidth = doubleSpanWidth;
      } else {
        maxWidth = cols * (cellWidth + gap) - gap;
      }
      ...
      let newSpan = newWidth < spanThresholdWidth ? 1 : 2;
      newSpan = Math.max(1, Math.min(resizingItem.group === 'left' ? 2 : cols, newSpan));
  ...
  function handleEnd(e) {
    ...
    if (gestureItem && (!e.touches || e.touches.length === 0)) {
        ...
        let targetSpan = currentVisualWidth < spanThresholdWidth ? 1 : 2;
    ...
    if (resizingItem) {
      const el = resizingItem.el;
      const targetWidth =
        resizingItem.span === 1 ? cellWidth : doubleSpanWidth;
